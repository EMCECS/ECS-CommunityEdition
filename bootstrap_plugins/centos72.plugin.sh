...
list_prefix_packages='epel-release python-devel wget curl ntp git-perl'
...
# command to install one or more os package manager packages
in_repo_pkg() {
    sudo yum -y install $*
}

# command to remove one or more os package manager packages
rm_repo_pkg() {
