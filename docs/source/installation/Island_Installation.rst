...
migrated into a network-isolated environment and the ECS deployment will
then proceed from the install node.
...
rebooted if needed, then the next step is to simply run
``island-step1``.
...
base operating system packages and the ECS software Docker image. The
installer will stop after this step.

5. Migrate the Install Node
~~~~~~~~~~~~~~~~~~~~~~~~~~~

At this time, please shut down the install node VM and migrate it into
your isolated environment.

6. Deploying the Island Environment ECS Nodes (``island-step2``)
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

Once the install node has been migrated into your island, you can begin
deploying ECS by running ``island-step2``. The next tasks the installer
will perform are: configuring the ECS nodes, performing a pre-flight
check to ensure ECS nodes are viable deployment targets, distributing
the artifact cache to ECS nodes, installing necessary packages, and
finally deploying the ECS software and init scripts onto ECS nodes.
...
Once ``island-step2`` has completed, you may then direct the installer
to configure the ECS topology by running ``island-step3``. Once
``island-step3`` has completed, your ECS will be ready for use.
