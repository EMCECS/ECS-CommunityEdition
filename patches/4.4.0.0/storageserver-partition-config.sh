...
# Root the block bin volumes are mounted under, named <partitionroot>/uuid-<PARTUUID>.
...
then
    ...
    echo "commodity node detected - describing volumes under ${partitionroot}"

    if [ ! -e "$p_configfile" ]; then
        echo "creating $p_configfile"
        mkdir -p "$configdir"

        volume_prefix="${partitionroot}/uuid-"
        volume_prefix_length=${#volume_prefix}
        separator=""
