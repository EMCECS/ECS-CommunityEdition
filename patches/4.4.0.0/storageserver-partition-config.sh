...
# Root path where block bin volumes are mounted, named uuid-<PARTUUID>.
# NOTE: _fix_ss_partitionroot() in boot_datasvcs_lib.sh uses a greedy
# sed (s~partitionroot.*~partitionroot=VALUE~g) that corrupts any line
# containing "partitionroot" followed by other text.  We therefore copy
# the value into $_pr immediately and use that alias everywhere else.
partitionroot=${STORAGESERVER_PARTITION_CONFIG__PARTITIONROOT:-/dae}
_pr=$(grep '^partition''root=' "$configfile" 2>/dev/null | head -1 | cut -d= -f2-)
: "${_pr:=$STORAGESERVER_PARTITION_CONFIG__PARTITIONROOT}"
: "${_pr:=/dae}"
...
# Write the configuration unless it already holds settings, so that an empty or
# placeholder file is treated as absent.
if ! grep -qE '^[[:alnum:]_]+=' "$configfile" 2>/dev/null; then
...
then
    ...
    # No agent is available to supply partition information on CE, so describe
    # the volumes mounted under the partition root instead.
    echo "commodity node detected - describing volumes under ${_pr}"

    if [ ! -e "$p_configfile" ]; then
        echo "creating $p_configfile"
        mkdir -p "$configdir"

        volume_prefix="${_pr}/uuid-"
        volume_prefix_length=${#volume_prefix}
        separator=""
